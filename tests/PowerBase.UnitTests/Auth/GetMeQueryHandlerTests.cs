...
public class GetMeQueryHandlerTests
{
    ...
    private GetMeQueryHandler CreateSut() => new(_userRepo, _queryContext);

    [Fact]
    public async Task HandleAsync_AuthenticatedUser_ReturnsUser()
    {
        var user = new User { Id = 5, PublicId = Guid.NewGuid(), Email = "me@example.com", Name = "Alice" };
        _queryContext.UserId.Returns(5L);
        _userRepo.GetByIdAsync(5L).Returns(user);
        var sut = CreateSut();

        var result = await sut.HandleAsync(new GetMeQuery());
    ...
    public async Task HandleAsync_UnauthenticatedUser_ThrowsUnauthorizedActionException()
    {
        ...
        await sut.Invoking(s => s.HandleAsync(new GetMeQuery()))
