...
public class RecordHandlerTests
{
    ...
    public async Task CreateRecord_ValidCommand_ReturnsRecordResult()
    {
        ...
        _recordRepo.CreateAsync(Arg.Any<AppTable>(), Arg.Any<IReadOnlyList<AppField>>(), Arg.Any<IReadOnlyDictionary<long, object?>>(), Arg.Any<IDbTransaction>(), Arg.Any<CancellationToken>(), Arg.Any<Action<PowerBase.Application.Common.Models.SearchIndexMessage>?>())
            .Returns(publicId);
        var sut = new CreateRecordCommandHandler(_tableRepo, _fieldRepo, _recordRepo, _relRepo, _enforcer, _auditRepo, _triggerInterceptor, _uow, _queryContext, _userRepo, Substitute.For<IMessagePublisher>(), _engine);
    ...
    public async Task UpdateRecord_ValidFieldValues_CallsUpdateOnRepo()
    {
        ...
        _tableRepo.GetByPublicIdAsync(table.PublicId).Returns(table);
        _fieldRepo.ListByTableAsync(table.Id).Returns(new List<AppField> { field });
        IRecordWriteService writeService = new RecordWriteService(_tableRepo, _fieldRepo, _recordRepo, _relRepo, _appUserRepo, _userRepo, _auditRepo, _triggerInterceptor, _engine);
