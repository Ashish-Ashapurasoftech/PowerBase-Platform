...
public class FieldHandlerTests
{
    ...
    private CreateFieldCommandHandler MakeSut() =>
        new(_tableRepo, _fieldRepo, _fieldTypeRepo, _schemaEngine, _queryContext, _auditRepo, _formRepo, _settingsRegistry);
    ...
    public async Task CreateField_ValidCommand_CreatesFieldAndUpdatesPhysicalName()
    {
        ...
        var result = await MakeSut().HandleAsync(new CreateFieldCommand(table.PublicId, "Text", "Email", null, null, false));
    ...
    public async Task CreateField_DuplicateName_ThrowsDuplicateException()
    {
        ...
        await MakeSut().Invoking(s => s.HandleAsync(new CreateFieldCommand(table.PublicId, "Text", "Email", null, null, false)))
    ...
    {
        await MakeSut().Invoking(s => s.HandleAsync(new CreateFieldCommand(Guid.NewGuid(), "Blob", "File", null, null, false)))
    ...
    {
        await MakeSut().Invoking(s => s.HandleAsync(new CreateFieldCommand(Guid.NewGuid(), "Text", "", null, null, false)))
            .Should().ThrowAsync<ValidationException>();
    }
